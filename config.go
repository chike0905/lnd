...
type chainConfig struct {
	...
	BsafeNet bool `long:"bsafenet" description:"Use the my network"`
...
func loadConfig() (*config, error) {
	...
	case cfg.Bitcoin.Active:
		...
		if cfg.Bitcoin.BsafeNet {
			numNets++
			activeNetParams = bitcoinBsafeNetParams
		}
		...
		if numNets > 1 {
			str := "%s: The mainnet, testnet, bsafenet, regtest, and " +
		...
		if numNets == 0 {
			str := "%s: either --bitcoin.mainnet, or " +
				"bitcoin.testnet, bitcoin.simnet, bitcoin.bsafenet, or bitcoin.regtest " +
				"must be specified"
			err := fmt.Errorf(str, funcName)
			return nil, err
		}
